...
public class RestResponseExceptionHandler extends ResponseEntityExceptionHandler {
  ...
  private static final Logger logger =
      Logger.getLogger(RestResponseExceptionHandler.class.getName());

  @ExceptionHandler(value = {Exception.class})
  protected ResponseEntity<ErrorResponse> handleUnknownException(Exception e) {
    ErrorResponse errorResponse =
        new ErrorResponse(HttpStatus.INTERNAL_SERVER_ERROR, "Unknown error happened");
    logger.log(Level.SEVERE, e.getMessage(), e.getCause());
    return new ResponseEntity<>(errorResponse, errorResponse.status());
  }

  @ExceptionHandler(value = {TaxRecordNotFoundException.class})
  protected ResponseEntity<ErrorResponse> handleTaxRecordNotFoundException(
      TaxRecordNotFoundException e) {
